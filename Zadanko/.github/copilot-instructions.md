...
Use this file as the quick-start behavior guide for AI coding agents in this repository.

## Current state

- The application scaffold exists in `.bootstrap-scaffold/`.
- Bootstrap verification and failure details are recorded in [context/changes/bootstrap-verification/verification.md](../context/changes/bootstrap-verification/verification.md).
- Product and delivery context live in [context/foundation/prd.md](../context/foundation/prd.md) and [context/foundation/tech-stack.md](../context/foundation/tech-stack.md).

## Read first

1. [../.bootstrap-scaffold/CLAUDE.md](../.bootstrap-scaffold/CLAUDE.md)
2. [../.bootstrap-scaffold/README.md](../.bootstrap-scaffold/README.md)
3. [../context/foundation/prd.md](../context/foundation/prd.md)
4. [../context/changes/bootstrap-verification/verification.md](../context/changes/bootstrap-verification/verification.md)

Link to these docs instead of copying long sections into generated outputs.

## Core commands

Run from `.bootstrap-scaffold/` unless told otherwise.
...
- Astro SSR app with React islands, Tailwind, Supabase auth, Cloudflare deployment.
- Prefer Astro components for static content; use React only for interactive islands.
- Auth and protected-route behavior are centralized in `src/middleware.ts`.
- Shared helpers and services belong in `src/lib/`; shared types in `src/types.ts`.
- API routes should validate input and use explicit HTTP method exports.

## Project conventions

- Use import alias `@/*` for `src/*` paths.
- Use `cn()` utility for class merging rather than manual class-string concatenation.
- Keep changes small and consistent with existing patterns in `.bootstrap-scaffold/src/`.
- Respect generated formatting from Prettier and Tailwind plugin ordering.

## Known environment pitfalls

- On Windows PowerShell, `npm` can fail due to execution policy (`npm.ps1` blocked).
- If that happens, run `npm.cmd` or adjust execution policy before retrying bootstrap/install steps.
- Node version target is documented in `.bootstrap-scaffold/.nvmrc`.
- Local Supabase requires Docker and configured env values.

## Safety rules for this repo

- Do not write to `context/archive/`.
- Keep `context/` as workflow history and source-of-truth docs.
- Do not delete or overwrite historical records in `context/changes/`.

## Suggested workflow for agents

1. Read the four files listed in "Read first".
2. Confirm local toolchain and env variables.
3. Implement minimal scoped change in `.bootstrap-scaffold/`.
4. Run lint/build checks relevant to the change.
